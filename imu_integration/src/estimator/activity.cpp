...
#include "../tools/file_manager.cpp"
...
Activity::Activity(void) 
    ...
    G_(0, 0, -9.81), // dk change sign of g
...
void Activity::Init(void) {
    ...
    private_nh_.param("imu/gravity/z", imu_config_.gravity.z, -9.81); // dk change sign of g
    ...
    //private_nh_.param("pose/topic_name/ground_truth", odom_config_.topic_name.ground_truth, std::string("/pose/ground_truth")); //"/sim/sensor/pos"
    private_nh_.param("pose/topic_name/ground_truth", odom_config_.topic_name.ground_truth, std::string("/sim/sensor/pos")); //"/sim/sensor/pos"
...
bool Activity::ReadData(void) {
    ...
    if (!initialized_) {
        ...
        if (static_cast<size_t>(0) == odom_data_buff_.size())
            return false; //dk change flase to true
    }
...
bool Activity::HasData(void) {
    ...
    if (
        ...
        return false; //dk change false to true
    }
...
bool Activity::UpdatePose(void) {
    if (!initialized_) {
        ...
        //
        // get deltas:
        Eigen::Vector3d angelDelta = Eigen::Vector3d::Zero();
        GetAngularDelta(1, 0, angelDelta);

        // update orientation:
        Eigen::Matrix3d R_curr = Eigen::Matrix3d::Zero();
        Eigen::Matrix3d R_prev = Eigen::Matrix3d::Zero();
        UpdateOrientation(angelDelta, R_curr, R_prev);
...
inline Eigen::Vector3d Activity::GetUnbiasedLinearAcc(
    ...
    const Eigen::Matrix3d &R
        ...
        //LOG(INFO) << "imu after correction acc is: " << R*(linear_acc - linear_acc_bias_) - G_ << std::endl;
        return R*(linear_acc - linear_acc_bias_); //R*(linear_acc - linear_acc_bias_) -G_
}
...
bool Activity::GetAngularDelta(
    ...
    //angular_delta = delta_t * angular_vel_prev; //dk add
...
bool Activity::GetVelocityDelta(
    ...
    //velocity_delta = delta_t * linear_acc_prev; //dk add
...
void Activity::UpdatePosition(const double &delta_t, const Eigen::Vector3d &velocity_delta) {
    ...
    //pose_.block<3, 1>(0, 3) += delta_t*vel_; //dk add
    vel_ += velocity_delta;
}
...
bool Activity::SaveTrajectory() {
    ...
    if (!is_file_created) {
        if (!FileManager::CreateDirectory("/home/xkhuskit/xk_workspaces/workspace/assignments/05-imu-navigation/src/imu_integration/src/trajectory_data"))
            return false;
        if (!FileManager::CreateFile(ground_truth, "/home/xkhuskit/xk_workspaces/workspace/assignments/05-imu-navigation/src/imu_integration/src/trajectory_data/ground_truth.txt"))
            return false;
        if (!FileManager::CreateFile(laser_odom, "/home/xkhuskit/xk_workspaces/workspace/assignments/05-imu-navigation/src/imu_integration/src/trajectory_data/odometry.txt"))
            return false;
    ...
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 4; ++j) {
            OdomData odom_data_ = odom_data_buff_.front();
            gt_odometry_ = odom_data_.pose;
            ground_truth << gt_odometry_(i, j);
